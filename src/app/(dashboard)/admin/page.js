 ...
 * SyncJudgmentsPanel
 * ------------------
 * Allows admins to manually trigger the Apify court-judgment sync without
 * waiting for the automated Vercel cron (which runs every 3 days).
 * Calls POST /api/sync-judgments and displays the resulting statistics.
 */
function SyncJudgmentsPanel() {
  const [syncing, setSyncing] = useState(false);
  const [result, setResult]   = useState(null); // last sync stats
  const [error, setError]     = useState(null);

  const handleSync = async () => {
    if (
      !confirm(
        "Trigger a live Apify scrape of all Pakistani court websites?\n\n" +
        "This may take up to 5 minutes and will consume Apify compute credits.",
      )
    )
      return;
    ...
    try {
      const data = await api.post("/api/sync-judgments", {});
      setResult(data);
      toast.success(
        `Sync complete — ${data.inserted ?? 0} new, ${data.updated ?? 0} updated.`,
      );
    } catch (err) {
      setError(err.message || "Sync failed.");
      toast.error("Judgment sync failed. Check console for details.");
    } finally {
      setSyncing(false);
    }
  };
  ...
  return (
    <div className="card p-5 border-indigo-100">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <h3 className="font-bold text-slate-800 text-sm">
            Sync Court Judgments (Apify)
          </h3>
          <p className="text-xs text-slate-400">
            Scrapes SCP, LHC, IHC, PHC, and BHC via Apify and upserts results
            into MongoDB. Runs automatically every 3 days via Vercel cron.
          </p>
        </div>
      </div>
      ...
      <button
        onClick={handleSync}
        disabled={syncing}
        className="btn-secondary text-sm gap-2 border-indigo-200 text-indigo-700 hover:bg-indigo-50"
      >
        {syncing ? (
          <RefreshCw className="w-4 h-4 animate-spin" />
        ) : (
        ...
        {syncing ? "Syncing… (may take ~5 min)" : "Run Sync Now"}
      </button>

      {/* Result stats */}
      {result && (
        <div className="mt-3 p-3 rounded-lg bg-green-50 border border-green-100 text-xs text-green-800 space-y-0.5">
          ...
          <p>Skipped / invalid: <strong>{result.skipped}</strong></p>
          {result.errors > 0 && (
            <p className="text-amber-700">Partial errors: {result.errors}</p>
          )}
