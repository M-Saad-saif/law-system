...
import { EmptyState, PageLoader, ConfirmDialog, Modal, SearchInput, Spinner } from '@/components/ui';
import { BookOpen, Upload, Trash2, ExternalLink, FileText, Search, X } from 'lucide-react';
...
export default function BooksPage() {
  ...
  const fetchBooks = useCallback(async () => {
    try {
      const data = await api.get(`/api/books${search ? `?search=${encodeURIComponent(search)}` : ''}`);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <SearchInput value={search} onChange={setSearch} placeholder="Search by title or author..." className="max-w-md" />
      ...
      {books.length === 0 ? (
        <div className="card">
          <EmptyState
            ...
            description={search ? 'Try a different search.' : 'Upload PDF law books to build your library.'}
            action={!search && (
          ...
          {books.map((book) => (
            <div key={book._id} className="card-hover p-4 flex flex-col gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-800 text-sm leading-snug truncate">{book.name}</h3>
                {book.author && <p className="text-xs text-slate-500 mt-0.5 truncate">{book.author}</p>}
                {book.description && <p className="text-xs text-slate-400 mt-1 line-clamp-2">{book.description}</p>}
                {book.fileSize && <p className="text-xs text-slate-400 mt-1">{fileSizeLabel(book.fileSize)}</p>}
              </div>
      ...
      {/* Upload Modal */}
      <Modal isOpen={showUpload} onClose={() => { setShowUpload(false); setFile(null); }} title="Upload PDF Book" size="sm">
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          >
            ...
            {file ? (
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 font-medium">Drop a PDF here or click to browse</p>
          ...
          <div className="form-group">
            ...
            <input className="input" placeholder="e.g. PPC 1860 Annotated" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <textarea className="textarea" rows={2} placeholder="Brief description..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
          </div>
      ...
      {viewingBook && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex flex-col">
          <div className="flex items-center justify-between bg-surface-900 px-5 py-3 border-b border-white/10">
            <div>
              <h3 className="text-white font-semibold text-sm">{viewingBook.name}</h3>
              {viewingBook.author && <p className="text-slate-400 text-xs">{viewingBook.author}</p>}
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setViewingBook(null)} className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/10">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
