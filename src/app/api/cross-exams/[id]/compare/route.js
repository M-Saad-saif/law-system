...
export const GET = withAuth(async (req, { params }, user) => {
  ...
  if (!snapA)
    return NextResponse.json(
      { error: `Version ${vA} not found.` },
      { status: 404 },
    );
  if (!snapB)
    return NextResponse.json(
      { error: `Version ${vB} not found.` },
      { status: 404 },
    );
  ...
  for (const wId of allWitnessIds) {
    ...
    const allQaIds = new Set([
      ...Object.keys(wA.qaPairs),
      ...Object.keys(wB.qaPairs),
    ]);
    const qaDiffs = [];
  ...
  return NextResponse.json({
    versionA: {
      version: vA,
      message: snapA.message,
      createdAt: snapA.createdAt,
    },
    versionB: {
      version: vB,
      message: snapB.message,
