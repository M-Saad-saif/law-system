...
export const PUT = withAuth(async (req, { params }, user) => {
  ...
  if (isReviewer) {
    if (body.editedQuestion   !== undefined) qaPair.editedQuestion   = body.editedQuestion;
    if (body.editedAnswer     !== undefined) qaPair.editedAnswer     = body.editedAnswer;
    if (body.useEditedVersion !== undefined) qaPair.useEditedVersion = body.useEditedVersion;
  }
  ...
  if (isCreator && ['draft', 'changes_requested'].includes(exam.status)) {
    if (body.originalQuestion !== undefined) qaPair.originalQuestion = body.originalQuestion;
    if (body.originalAnswer   !== undefined) qaPair.originalAnswer   = body.originalAnswer;
  }
