...
import {
  Scale, LayoutDashboard, FolderOpen, CalendarDays,
  BookOpen, Bell, Settings, LogOut, ChevronRight
} from 'lucide-react';
      ...
      <div className="px-3 pb-4 border-t border-white/5 pt-3">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1">
          ...
          <div className="min-w-0">
            <div className="text-white text-xs font-semibold truncate">{user?.name || 'Loading...'}</div>
            <div className="text-slate-500 text-[10px] truncate capitalize">{user?.role || 'lawyer'}</div>
          </div>
        </div>
        <button
          onClick={logout}
