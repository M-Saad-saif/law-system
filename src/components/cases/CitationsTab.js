...
export default function CitationsTab({ caseId, citations, onUpdate }) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await api.delete(`/api/cases/${caseId}/citations`, {
        citationId: deleteTarget._id,
      });
    ...
    <div className="space-y-4">
      ...
      {citations.length === 0 ? (
        <div className="card">
          <EmptyState
            icon={BookMarked}
            title="No citations added"
            description="Add relevant legal citations for this case."
          />
        </div>
      ) : (
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          ...
          <div className="form-group">
            ...
            <textarea
              className="textarea"
              rows={3}
              placeholder="Key legal principle or holding..."
              value={form.text}
